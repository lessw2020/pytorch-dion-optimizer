...
from dion_optimizer import create_dion_optimizer, DionOptimizer, get_parameter_info
...
class TrainingLogger:
    ...
    def plot_comparison(self, save_path: Path):
        ...
        for opt_name, metric_list in self.metrics.items():
            ...
            # Loss vs wall time
            axes[0, 1].plot(
                wall_times, losses, label=f"{opt_name}", color=color, alpha=0.8
            )
            ...
            # Gradient norms
            axes[1, 0].plot(
                steps, grad_norms, label=f"{opt_name}", color=color, alpha=0.8
            )
            ...
            # Memory usage
            axes[1, 1].plot(
                steps, memory_usage, label=f"{opt_name}", color=color, alpha=0.8
            )
        ...
        plt.tight_layout()
        plt.savefig(
            save_path / f"{self.experiment_name}_comparison.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
...
def run_comparison_experiment(config: TrainingConfig, save_dir: Path = Path("results")):
    ...
    print(
        f"Dion optimizer - Matrix params: {len(optimizer_dion.matrix_params)}, "
    ...
    train_with_optimizer(
        model_adamw, optimizer_adamw, dataloader, config, "adamw", logger
    )
    ...
    if dion_metrics and adamw_metrics:
        ...
        print(
            f"  Improvement: {((adamw_final_loss - dion_final_loss) / adamw_final_loss * 100):+.1f}%"
        )
...
def main():
    ...
    parser.add_argument(
        "--max_steps", type=int, default=3000, help="Maximum training steps"
    )
    parser.add_argument(
        "--dion_lr", type=float, default=0.01, help="Dion learning rate"
    )
    parser.add_argument(
        "--adamw_lr", type=float, default=0.0002, help="AdamW learning rate"
    )
    parser.add_argument(
        "--rank_factor", type=float, default=0.25, help="Dion rank factor"
    )
